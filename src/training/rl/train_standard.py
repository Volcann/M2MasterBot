...
class NoTeacherTrainer:
        ...
        while True:
            ...
            if not self.ui.game_is_over and self.ui.input_column is None:
                action = self.agent.select_action(
                    matrix, self.ui.next_value, self.epsilon
                )
                ...
                vol = self.agent.update_q_learning(
                    state_features, reward, new_matrix, next_val, False
                )
            ...
            try:
                _, current_matrix = remove_redundant(
                    matrix=current_matrix, remove_values=remove_values
                )
            except Exception:
                pass
            while True:
    ...
    def run(self):
        for ep in range(self.episodes):
            ...
            print(
                f"Episode {ep + 1}/{self.episodes} Sparse Reward: {reward} Epsilon: {self.epsilon:.3f}"
            )
        self.agent.save(self.output_path)
...
def run_no_teacher_training():
    parser = argparse.ArgumentParser(
        description="No-Teacher RL Training (Visualize Failure Cycle)"
    )
    parser.add_argument("--episodes", type=int, default=200, help="Number of episodes")
    parser.add_argument("--fps", type=int, default=120, help="Training speed")
    parser.add_argument(
        "--lr", type=float, default=0.1, help="Learning rate (high to trigger collapse)"
    )
    args = parser.parse_args()
    trainer = NoTeacherTrainer(
        episodes=args.episodes, fps=args.fps, learning_rate=args.lr
    )
    trainer.run()
