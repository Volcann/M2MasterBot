...
class NoTeacherVisualizer:
    ...
    def draw(self, screen, theta, history, epsilon, weight_volatility, x_offset):
        ...
        title = self.header_font.render(
            "NO-TEACHER AGENT (SCRATCH)", True, (255, 100, 0)
        )
        screen.blit(title, (x_offset + 20, y))
        y += 40
        eps_label = self.font.render(
            f"EXPLORATION (ε): {epsilon:.3f}", True, (150, 150, 160)
        )
        screen.blit(eps_label, (x_offset + 20, y))
        y += 25
        pygame.draw.rect(screen, (40, 40, 45), (x_offset + 20, y, 360, 15))
        pygame.draw.rect(
            screen, (255, 200, 0), (x_offset + 20, y, int(360 * epsilon), 15)
        )
        y += 35
        ...
        y += 60
        volt_title = self.header_font.render(
            "WEIGHT VOLATILITY (ΔW)", True, (255, 255, 255)
        )
