...
class Debugger:
    def __init__(self, feature_keys):
        ...
        self._draw_counter = 0  # throttle full redraws
        ...
        # left: big radar circle spanning all rows
        self.ax_radar   = self.fig.add_subplot(gs[:, 0], polar=True)
        # right: three stacked square panels
    ...
    def _init_radar(self):
        ...
        # persistent line — updated in-place each frame
        self._radar_line, = ax.plot(
            angles, [0.0] * (self.n + 1),
            color=ACCENT, linewidth=2.2, linestyle='solid', zorder=3
        )
        # persistent fill polygon — updated via set_xy
        self._radar_fill, = ax.fill(
            angles, [0.0] * (self.n + 1),
            color=ACCENT, alpha=0.18, zorder=2
        )
        # single scatter for all feature dots — update offsets each frame
        self._radar_dots = ax.scatter(
            angles[:-1], [0.0] * self.n,
            c=FEATURE_COLORS[:self.n], s=55, zorder=4, edgecolors='none'
        )
    ...
    def _init_history(self):
        ...
        self._hscore_line, = ax.plot(
            [], [],
            label='h-score', color=GOLD,
            linewidth=2.5, linestyle='--', alpha=0.9
        )

        ax.set_title(
            'CONTRIBUTION  HISTORY',
            color=ACCENT, fontsize=9, fontweight='bold', pad=10
        )
        ax.legend(
    ...
    def update(self, column, contributions, total_heuristic):
        ...
        # full redraw every 3 moves — flush_events every move to stay responsive
    ...
    def _redraw_radar(self, contributions):
        ...
        # update fill polygon in-place using polar (theta, r) coords
        xy = np.column_stack([self._radar_angles_np, np.array(norm_closed)])
        self._radar_fill.set_xy(xy)

        # update dot positions via scatter offsets
        angles_arr = np.array(self._radar_angles[:-1])
        # convert polar to cartesian for scatter (polar axes handle this)
    ...
    def _redraw_history(self):
        ...
        # only rescale every 5 frames — cheap skip
    ...
    def draw_summary(self, summary_data, best_column):
        ...
        for d in summary_data:
            ...
            if d['col'] == best_column:
                row_colors.append(['#003300', '#004400', '#004400', '#003300', '#004400'])
            elif rank == 1:
                row_colors.append([PANEL] * 5)
            else:
                row_colors.append(['#0d1117'] * 5)

        col_widths = [0.08, 0.18, 0.18, 0.14, 0.10]
        ...
        self.ax_bars.set_title(
            ...
            if best_column < len(summary_data) else
